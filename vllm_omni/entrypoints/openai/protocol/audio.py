...
class OpenAICreateSpeechRequest(BaseModel):
    ...
    return_codec_tokens: bool = Field(
        default=False,
        description=(
            "If true, return a JSON response containing both the base64-encoded audio "
            "and the raw codec tokens (shape [T, Q]) generated by the talker. "
            "Only supported for non-streaming requests with TTS models that produce "
            "intermediate codec tokens (e.g. Qwen3-TTS)."
        ),
    )
...
class AudioWithCodesResponse(BaseModel):
    """Response containing both audio bytes and raw codec tokens.

    Used when the request sets ``return_codec_tokens=True``. The codec
    tokens are the [T, Q] integer sequence produced by the talker stage
    (for Qwen3-TTS: Q=16 codebooks at 12 Hz or 25 Hz).
    """

    audio_data: str = Field(description="Base64-encoded audio bytes (format determined by response_format).")
    media_type: str = Field(description="MIME type of the audio data (e.g. 'audio/wav').")
    codec_tokens: list[list[int]] = Field(description="Codec tokens, shape [T, Q].")
